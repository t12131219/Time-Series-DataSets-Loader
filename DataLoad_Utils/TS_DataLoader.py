...
# def load_from_tsfile_to_dataframe(full_file_path_and_name, return_separate_X_and_y=True,
#                                   replace_missing_vals_with='NaN'):
#     """Loads data from a .ts file into a Pandas DataFrame with timestamps as a column.
#
#     Modified version that stores timestamps as a separate column instead of using them as index.
#     """
#
...
#                     else:
#                         raise TsFileParseException("invalid classLabel value")
#                     if len(tokens) == 2 and class_labels:
#                         raise TsFileParseException("if the classlabel tag is true then class values must be supplied")
#                     has_class_labels_tag = True
#                     class_label_list = [token.strip() for token in tokens[2:]]
#                     metadata_started = True
...
#                                         instance_list.append([])
#
#                                     # Store as dictionary with timestamps and values
#                                     instance_list[this_line_num_dimensions].append({
#                                         'timestamps': timestamps_for_dimension,
#                                         'values': values_for_dimension
#                                     })
#                                     this_line_num_dimensions += 1
#
#                                     has_another_value = False
...
#                                                     line_num + 1) + " contains tuples where the timestamp format is inconsistent")
#
#                                         timestamps_for_dimension.append(timestamp)
#                                         values_for_dimension.append(value)
#
#                                         if previous_timestamp_was_timestamp is None and timestamp_is_timestamp:
#                                             previous_timestamp_was_timestamp = True
#                                             previous_timestamp_was_int = False
...
#             raise TsFileParseException("file contained metadata but no data")
#
#         # Reconstruct data with timestamps as columns
#         expanded_data = []
#
#         # Determine number of cases (rows in original data)
#         num_cases = len(instance_list[0]) if num_dimensions > 0 else 0
#
#         for case_idx in range(num_cases):
#             # Get all timestamps (assuming they're the same across dimensions for a case)
#             if timestamps:
#                 timestamps = instance_list[0][case_idx]['timestamps']
#
#                 for ts_idx, timestamp in enumerate(timestamps):
#                     # row_data = {'date': timestamp}
#                     row_data = {'timestamp': timestamp}
#
#                     # Add all dimension values for this timestamp
#                     for dim in range(num_dimensions):
#                         dim_values = instance_list[dim][case_idx]['values']
#                         if ts_idx < len(dim_values):
#                             row_data[f'dim_{dim}'] = dim_values[ts_idx]
#                         else:
#                             row_data[f'dim_{dim}'] = np.nan
#
#                     # Add class value if present
#                     if target_labels and case_idx < len(class_val_list):
#                         row_data['class_val'] = class_val_list[case_idx]
#
#                     expanded_data.append(row_data)
#
#             else:
#                 for case_idx in range(num_cases):
#                     # 如果没有 'timestamps'，使用默认的整数索引作为时间戳
#                     num_timestamps = len(instance_list[0][case_idx]['values'])
#                     timestamps = list(range(num_timestamps))
#
#                     for ts_idx, timestamp in enumerate(timestamps):
#                         row_data = {'Idx': timestamp}
#
#                         # 添加所有维度的值
#                         for dim in range(num_dimensions):
#                             dim_values = instance_list[dim][case_idx]['values']
...
#         if target_labels:
#             if return_separate_X_and_y:
#                 y = expanded_df['class_val'].values
#                 expanded_df.drop('class_val', axis=1, inplace=True)
#                 return expanded_df, y
#             else:
#                 return expanded_df
#         else:
#             return expanded_df
...
def load_from_tsfile_to_dataframe(full_file_path_and_name, return_separate_X_and_y=True,
                        ...
                        for dim in range(0, num_dimensions):
                            ...
                            if dimension:
                                ...
                                instance_list[dim].append({
                                    'timestamps': list(range(len(data_series))),
                                    'values': data_series
                                })
                            else:
                                instance_list[dim].append({
    ...
    if line_num:
        ...
        if TimeStamp:
            ...
            expanded_df = expanded_df.rename(columns={'timestamp': 'Index'})
...
def convert_to_time_series(df):
    """
    将多维时间序列数据转换为一个有序的时序 DataFrame，其中第一列是时间戳。

    参数:
    - df: 输入的 DataFrame，其中每一列是一个时间序列数据，索引是时间戳。

    返回:
    - 一个有序的时序 DataFrame，其中第一列是时间戳，其余列是原始数据。
    """
    ...
    for col in df.columns:
        # 提取时间戳和值
        time_series = df[col].reset_index()
        time_series.columns = ['timestamp', col]

        # 如果是第一列，直接赋值
        if result_df.empty:
            result_df = time_series
        else:
            # 合并时间戳，处理缺失值
            result_df = pd.merge(result_df, time_series, on='timestamp', how='outer')
...
def sort_timeseries(df):
    """
    将包含时间戳的DataFrame按照时间排序

    参数：
    df : DataFrame
        包含'timestamp'列和其他数值列的DataFrame

    返回：
    DataFrame
        按时间排序后的新DataFrame（原数据不会被修改）
    """
    # 创建副本避免修改原数据
    sorted_df = df.copy()

    has_timestamp = 'timestamp' in sorted_df.columns
    if not has_timestamp:
        return sorted_df

    # 转换时间戳列
    sorted_df['timestamp'] = pd.to_datetime(
        sorted_df['timestamp'],
        errors='coerce',  # 将无法解析的时间设为NaT
        format='%Y-%m-%d %H:%M:%S'  # 强制匹配完整时间格式
    )

    # 处理部分时间格式（如缺少秒）
    mask = sorted_df['timestamp'].isna()
    sorted_df.loc[mask, 'timestamp'] = pd.to_datetime(
        sorted_df.loc[mask, 'timestamp'],
        errors='coerce',
        format='%Y-%m-%d %H:%M'
    )

    # 删除无效时间戳的行
    valid_df = sorted_df.dropna(subset=['timestamp'])

    # 按时间排序
    sorted_df = valid_df.sort_values(by='timestamp')

    # 重置索引
    sorted_df = sorted_df.reset_index(drop=True)

    # 处理重复时间戳
    if sorted_df['timestamp'].duplicated().any():
        print("发现重复时间戳，保留第一个出现")
        sorted_df = sorted_df.drop_duplicates(subset=['timestamp'], keep='first')
